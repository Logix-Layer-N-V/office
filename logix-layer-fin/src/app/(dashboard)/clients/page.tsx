...
export default function ClientsPage() {
  ...
  const enrichedClients = mockClients.map(c => ({
    ...c,
    totalSpent: invoices.filter(inv => inv.clientId === c.id).reduce((s, inv) => s + (inv.total || 0), 0),
      ...
      <div className="p-6 space-y-4">
        <div className="grid grid-cols-4 gap-3">
          <div className="card p-3">
            ...
            <p className="text-lg font-semibold text-surface-800">{mockClients.length}</p>
          </div>
          <div className="card p-3">
        ...
        {/* Client Cards — links to detail page */}
        {view === "grid" ? (
            ...
            {filtered.map((client) => (
              <a key={client.id} href={`/clients/${client.id}`} className="card p-4 hover:border-brand-200 transition-colors block">
                ...
                <div className="space-y-1.5 mb-3">
                  ...
                  <div className="flex items-center gap-2 text-2xs text-surface-500"><Phone className="h-3 w-3" /> {client.phone}</div>
                  <div className="flex items-center gap-2 text-2xs text-surface-500"><MapPin className="h-3 w-3" /> {client.address}</div>
                </div>
